...
/**
 * Create a blank document from an embedded empty template (no user-supplied
 * file): txt/md are written as empty files, docx/xlsx/pptx as minimal-but-valid
 * OOXML packages generated by the backend. Fed through the same two-phase
 * commit pipeline as `commit` - Phase A is synchronous, so the new document +
 * its library copy exist once this resolves; the returned id is the Phase B
 * CreateBlank job (same compress work as Archive, labeled "creating" in the UI).
 * `format` is one of txt/md/docx/xlsx/pptx (case-insensitive). `aspect_ratio`
 * only affects pptx (`"16:9"` widens the slide; omitted keeps the 4:3 default).
 */
async function createBlankDocument(params: {
  name: string;
  format: string;
  aspect_ratio?: string;
}): Promise<string> {
  return invoke<string>("create_blank_document", params);
}
...
/**
 * Preview the document's live working copy (the library file that holds the
 * user's uncommitted edits), as opposed to [`previewVersion`] which exports the
 * committed archive snapshot. Use this when the document is "modified" so the
 * preview reflects the latest edits, not the last committed version. Outside
 * Tauri there is no backend to read from - returns null so the caller can show
 * "not supported" rather than crash in browser dev.
 */
async function previewWorkingCopy(params: {
  document_id: string;
}): Promise<ArrayBuffer | null> {
  if (!isTauri()) return null;
  return invoke<ArrayBuffer>("preview_working_copy", params);
}
...
async function subscribeJobs(
  ...
  return listen<RawJob>("job:update", (event) => {
    ...
    onUpdate?.(raw);
    // Archive (the async Phase B of a commit) and CreateBlank (the async Phase B
    // of creating a blank document) finalize a version's compress step; checkout
    // changes which version is current; delete removes a doc. All three change
    // the document list. Archive/CreateBlank add data and delete reclaims it, so
    // those also refresh the repo-size stat.
    const refreshKinds: RawJob["kind"][] = [
      "archive",
      "create_blank",
      "checkout",
      "delete",
